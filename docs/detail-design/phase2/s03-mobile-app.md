...
| Google SSO     | `@react-native-google-signin/google-signin` (Native) / Firebase `signInWithPopup` (Web) |
...
│   └─ Native: 「Google でサインイン」 → @react-native-google-signin/google-signin → signInWithCredential()
│     ↓
│   認証成功 → (tabs)/ にリダイレクト
│
└─ 認証済み → (tabs)/ を表示
...
- `signInWithGoogle`:
  - Web: Firebase Auth の `signInWithPopup` を使用（COOP ヘッダー問題を回避）。Emulator 接続時は Auth Emulator の認証UIが開く
  - Native: `@react-native-google-signin/google-signin` でネイティブ Google Sign-In UI を呼び出し、取得した `idToken` で `signInWithCredential` を呼び出す（ADR-010 参照）
...
- 認証: Native は `@react-native-google-signin/google-signin`（ADR-010）、Web は `signInWithPopup`（COOP 問題回避）
