...
- Firestore: `devices` コレクション（deviceId → parentIds 逆引き用、非正規化）
...
5. ペアリング済みデバイスからの `usageLogs` リクエストは、正しい `parentIds` で Firestore に保存される
6. 未登録デバイスからの `usageLogs` リクエストは 401 で拒否される
7. `users` コレクションに保護者情報とデバイス一覧が保存される
8. `devices` コレクションに deviceId → parentIds のマッピングが保存される
...
deviceId から parentIds への O(1) 逆引きのために、`users.childDevices` とは別に非正規化テーブルを設ける。
...
> **設計根拠**: `users.childDevices` は配列フィールドであり、Firestore の `array-contains` は完全一致が必要なため、deviceId 単体での逆引きクエリには適さない。`devices` コレクションを用いることで `usageLogs` 受信時の parentIds 取得を O(1) で実現する。
>
...
7. devices/{deviceId} を作成/更新 (parentIds, deviceName, registeredAt, lastSeenAt, syncAvailable)
...
4. 存在する → parentIds = devices/{deviceId}.parentIds
5. devices/{deviceId}.lastSeenAt を現在時刻に更新
6. usageLogs ドキュメントに正しい parentIds で upsert
```
