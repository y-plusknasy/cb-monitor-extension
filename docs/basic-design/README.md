...
│                     │                          │  - deviceId検証     │
│  - PWA/Browser検知  │   registerDevice         │  - parentIds逆引き  │
│  - 利用時間計測     │ ───────────────────────▶ │  - Firestoreへ保存  │
│  - バッチ送信       │                          │                     │
└─────────────────────┘                          │  [Scheduled]        │
                                                 │  - dailyLogs集計    │
...
| フィールド       | 型          | 説明                                                  |
| ---------------- | ----------- | ----------------------------------------------------- |
| (ドキュメントID) | —           | `{deviceId}_{date}_{appName}`                         |
| `parentIds`      | `string[]`  | 保護者の UID 配列                                     |
...
| `expireAt`       | `Timestamp` | TTL 自動削除用 (date + 30日)                          |
| `updatedAt`      | `Timestamp` | Firestore 側の最終更新日時                            |

**TTL**: 30日。日次サマリーとして利用。

**ドキュメントID 設計**: `{deviceId}_{date}_{appName}` とすることで、同一デバイス・同一日・同一アプリのログが 1 ドキュメントに集約される（upsert）。

> **Note**: 当初設計（分単位の詳細ログ）から ADR-001 で日次バッファ方式に変更。Extension が日別・アプリ別の累積秒数をローカルバッファで管理し、定期的に upsert する設計。

**インデックス:**

- `parentIds` (array-contains) + `date` ASC — 保護者ごとの日別クエリ
...
| `expireAt`       | `Timestamp` | TTL 自動削除用 (date + 84日)        |

**TTL**: 84日（約12週間）。日単位のサマリーとして長期保持。
...
| -------------------------------- | ------------ | ---------- | ------ | ----------------------------------------- |
| 利用時間サマリーカード（過去日） | `dailyLogs`  | 84日       | 日単位 | バッチ集計済みデータ                      |
...
#### `devices` コレクション（非正規化テーブル）

deviceId から parentIds への O(1) 逆引き用。
...
| `syncAvailable`  | `boolean \| null` | chrome.storage.sync の利用可否     |
...
| `usageLogs`    | 本人のみ   | API経由のみ | `auth.uid in resource.data.parentIds` で制限       |
| `dailyLogs`    | 本人のみ   | API経由のみ | `auth.uid in resource.data.parentIds` で制限       |
| `devices`      | なし       | API経由のみ | クライアント直接アクセス不可                       |
...
| コンポーネント               | 認証方式                     | 詳細                                                 |
| ---------------------------- | ---------------------------- | ---------------------------------------------------- |
| Chrome Extension → Functions | deviceId 検証                | Firestore `devices` コレクションに登録済みか確認     |
...
リブランディング・Extension UI 刷新・APK 配布パイプラインを整備し、エンドユーザーへの公開準備を行う。

| スプリント | 内容                                     | 成果物                                                                        |
| ---------- | ---------------------------------------- | ----------------------------------------------------------------------------- |
| S05        | リブランディング + Extension UI 刷新     | アプリ名「CB Link」統一、popup / 設定ページ UI 刷新、複数 OTP・デバイス名管理 |
| S06        | モバイルアプリ UI 改善 + ダークモード    | モバイル UI ブラッシュアップ、ダークモード対応（デザイン確定後に着手）        |
| S07        | APK ビルド・配布 + Chrome Store 公開準備 | QR コード付き APK 配布ページ、Chrome Store 向け README、ビルドパイプライン    |
...
| コスト最適化     | Firebase Functions のイベント駆動課金（アイドル時ゼロコスト）。Firestore TTL で `usageLogs` 30日 / `dailyLogs` 84日で自動削除。dailyLogs は日次バッチ集計で Firestore 書き込み回数を削減 |
