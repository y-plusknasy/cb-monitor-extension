...
deviceId から parentId への O(1) 逆引き用。
...
| S06        | APK ビルド・配布 + Chrome Store 公開準備 | QR コード付き APK 配布ページ、Chrome Store 向け README、ビルドパイプライン    |
| S07        | モバイルアプリ UI 改善 + ダークモード    | モバイル UI ブラッシュアップ、ダークモード対応（デザイン確定後に着手）        |
